...
describe('TodoList Test Suite', () => {
  ...
  describe('Filtering Todos by Due Date', () => {
    ...
    test('Should retrieve overdue items', () => {
        const initialCount = todos.overdue().length; // Store the initial count of overdue items
        todos.add({ title: 'Overdue Todo 2', dueDate: yesterday.toISOString().slice(0, 10), completed: false });
        expect(todos.overdue().length).toBe(initialCount + 1); // Check if the count of overdue items increased by 1
    });    
